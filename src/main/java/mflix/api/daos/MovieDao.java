...
public class MovieDao extends AbstractMFlixDao {
    ...
    public Document getMovie(String movieId) {
        ...
        List<Bson> subPipeline = new ArrayList<>();
        subPipeline.add(new Document("$match", new Document("$expr",new Document("$eq",Arrays.asList("$movie_id","$$id")))));
        subPipeline.add(new Document("$sort", new Document("date", -1)));
        Bson lookup = Aggregates.lookup("comments", let,
               subPipeline , "comments");
        pipeline.add(lookup);
